...
const flightId = process.argv[2] || "21b736e8", // example
  ...
  .then(json => {
    const data = json.result.response.data.flight,
      number = data.identification.callsign
    ...
    if (track) {
      ...
      let estate = geojson.features[0].properties.pop()
      console.log('НАЗВАНИЕ: ' + estate.number)
      console.log('КООРДИНАТЫ: ' + geojson.features[0].geometry.coordinates.pop())

      console.log('ВЫСОТА: ' + estate.altitude.meters) //ВЫСОТА
      console.log('СКОРОСТЬ: ' + estate.speed.kmh) //СКОРОСТЬ
      console.log('КУРС: '+ estate.heading) //КУРС
